...
def rest(method,table,params=None,body=None):
    ...
    if params: url += "?"+parse.urlencode(params,safe="(),.*")
...
def token():
    ...
    with request.urlopen(request.Request("https://oauth2.googleapis.com/token",data=body,headers={"Content-Type":"application/x-www-form-urlencoded"},method="POST"),timeout=25) as r:
...
def find_file(access,folder,name):
    ...
    params=parse.urlencode({"q":q,"fields":"files(id,name,webViewLink)","pageSize":"10"})
    data=drive("GET","https://www.googleapis.com/drive/v3/files?"+params,access)
    files=data.get("files") or []; return files[0] if files else None

def upload_or_update(access,folder,name,pdf):
    old=find_file(access,folder,name)
    if old:
        url=f"https://www.googleapis.com/upload/drive/v3/files/{old['id']}?uploadType=media&fields=id,name,webViewLink"
        data=drive("PATCH",url,access,pdf,"application/pdf"); file_id=str(data.get("id") or old["id"])
    else:
        boundary="rathodarchiveboundary"
        meta=json.dumps({"name":name,"parents":[folder]}).encode()
        body=(b"--"+boundary.encode()+b"\r\nContent-Type: application/json; charset=UTF-8\r\n\r\n"+meta+b"\r\n--"+boundary.encode()+b"\r\nContent-Type: application/pdf\r\n\r\n"+pdf+b"\r\n--"+boundary.encode()+b"--\r\n")
        data=drive("POST","https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,webViewLink",access,body,f"multipart/related; boundary={boundary}"); file_id=str(data.get("id") or "")
    if not file_id: raise RuntimeError("Drive did not return a file ID")
    return f"https://drive.google.com/file/d/{file_id}/view?usp=sharing"

def batch_window(now):
    start_day=((now.day-1)//4)*4+1
...
def extract(row):
    p=row.get("payload") if isinstance(row.get("payload"),dict) else {}
    q=str(p.get("question") or "").strip(); options=p.get("options")
    if not q or not isinstance(options,list) or len(options)!=4: return None
    try: correct=int(p.get("correct_index")) if p.get("correct_index") is not None else None
    except (TypeError,ValueError): correct=None
    return {"question":q,"options":[str(x) for x in options],"correct_index":correct,"explanation":str(p.get("explanation") or ""),"mode":str(p.get("mode") or "Quiz"),"source":str(p.get("source") or "App")}

def pdf_bytes(title,rows,answers):
    ...
    styles=getSampleStyleSheet(); styles.add(ParagraphStyle(name="RA",parent=styles["BodyText"],fontSize=8.5,leading=11)); styles.add(ParagraphStyle(name="RQ",parent=styles["Heading3"],fontSize=10.5,leading=14,spaceBefore=8,spaceAfter=4))
    story=[Paragraph(escape(title),styles["Title"]),Spacer(1,5*mm),Paragraph(escape(f"Questions: {len(rows)} | RATHOD HUB archive"),styles["RA"])]
    for n,q in enumerate(rows,1):
        story.append(Paragraph(escape(f"{n}. [{q['mode']}] {q['question']}"),styles["RQ"]))
        for i,opt in enumerate(q["options"]): story.append(Paragraph(escape(f"{chr(65+i)}. {opt}"),styles["RA"]))
        if answers:
            ans="Not available" if q["correct_index"] is None else chr(65+q["correct_index"])
            story.append(Paragraph(escape(f"Answer: {ans} | {q['explanation']}"),styles["RA"]))
    doc.build(story); return out.getvalue()

def rows_for(start,end):
    raw=rest("GET","rh_bridge_events",{"select":"id,event_type,payload,created_at","created_at":f"gte.{start.isoformat()}","order":"created_at.asc","limit":"2000"}) or []
    unique={}
    for row in raw:
        try: created=datetime.fromisoformat(str(row.get("created_at","")).replace("Z","+00:00"))
        except ValueError: continue
        if created>=end or daily(row) or row.get("event_type") not in {"quiz_question","quiz_item","question_solved"}: continue
        q=extract(row)
        if q: unique.setdefault((q["question"],q["mode"]),q)
    return list(unique.values())

def run_once():
    now=datetime.now(timezone.utc); start,end=batch_window(now); rows=rows_for(start,end)
    if not rows: log.info("No questions in %s-%s",start.date(),(end-timedelta(days=1)).date()); return
    access=token(); label=f"{start.strftime('%d-%b')}_to_{(end-timedelta(days=1)).strftime('%d-%b-%Y')}"
    if NOTES_FOLDER: log.info("Notes PDF: %s",upload_or_update(access,NOTES_FOLDER,f"Notes_{label}.pdf",pdf_bytes(f"RATHOD HUB Notes — {label}",rows,True)))
    if TESTS_FOLDER: log.info("Test PDF: %s",upload_or_update(access,TESTS_FOLDER,f"Test_{label}.pdf",pdf_bytes(f"RATHOD HUB Test Bank — {label}",rows,False)))
