...
def record_questions(topic: str, mode: str, questions: list[dict]) -> int:
    ...
    for q in questions:
        ...
        if not text or not isinstance(opts,list) or len(opts)!=4:
            continue
        rows.append({"event_type":"quiz_question","delivery_mode":"digest","display_name":"Telegram Quiz","payload":{"archive":True,"source":"Telegram","mode":str(mode or "Quiz"),"quiz_name":topic_text,"question":text,"options":[str(x) for x in opts],"correct_index":q.get("correct_index")}})
    if not rows:
        return 0
