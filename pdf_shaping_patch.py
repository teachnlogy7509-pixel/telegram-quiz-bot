...
replace_once("from urllib import parse,request", "from urllib import parse,request,error", "urllib error logging")
replace_once("TTFont(FONT_NAME,FONT_PATH)", "TTFont(FONT_NAME,FONT_PATH,shapable=True)", "Devanagari shaping")
replace_once(
...
replace_once('escape(f"Questions: {len(rows)} | RATHOD HUB archive")', 'escape(f"कुल प्रश्न: {len(rows)} | RATHOD HUB संग्रह")', "Hindi question label")
replace_once('escape(f"{chr(65+i)}. {o}")', 'escape(f"{(\'क\',\'ख\',\'ग\',\'घ\')[i]}. {o}")', "Hindi option labels")
replace_once(
...
replace_once('make_pdf("RATHOD HUB Notes - "+label,rows,True)', 'make_pdf("RATHOD HUB नोट्स - "+label,rows,True)', "Hindi notes title")
replace_once('make_pdf("RATHOD HUB Test Bank - "+label,rows,False)', 'make_pdf("RATHOD HUB टेस्ट प्रश्नपत्र - "+label,rows,False)', "Hindi test title")
replace_once(
