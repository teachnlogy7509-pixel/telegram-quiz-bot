...
def _rpc(name: str, payload: dict) -> dict:
    ...
    try:
        ...
        return {"success": False, "error": f"Supabase HTTP {exc.code}", "detail": detail}
...
def record_answer(telegram_user_id: int, chat_id: int, username: str, name: str, is_correct: bool, topic: str, correct_score: int = 20, wrong_score: int = -10) -> dict:
    ...
    # normal poll scoring working until the scoring-v2 SQL migration is run.
    if not result.get("success") and "HTTP 404" in str(result.get("error", "")):
        logger.warning("Scoring-v2 RPC unavailable; retrying legacy Telegram score RPC")
