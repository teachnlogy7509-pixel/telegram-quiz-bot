...
async def archivepdf(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Return the newest archive Notes and Test PDFs as separate Telegram buttons."""
    if not await asyncio.to_thread(master_control.is_enabled, "sakhi"): return
    ...
    buttons=[]
    if notes_url: buttons.append([InlineKeyboardButton("📘 Notes PDF खोलें",url=notes_url)])
    if test_url: buttons.append([InlineKeyboardButton("📝 Test PDF खोलें",url=test_url)])
    if not buttons:
        await message.reply_text("Latest archive record मिला, लेकिन PDF links अभी ready नहीं हैं।")
        return
    await message.reply_text("📚 <b>RATHOD Quiz Archive</b>\n🗓️ " + label + "\n\nनीचे दोनों PDFs अलग-अलग buttons में हैं।\n\n<i>इन links को group members खोल सकते हैं।</i>",parse_mode="HTML",reply_markup=InlineKeyboardMarkup(buttons),disable_web_page_preview=True)
...
async def post_init(app: Application) -> None:
    ...
    if app.job_queue:
        app.job_queue.run_repeating(gated_poll_job, interval=base.POLL_SECONDS, first=8, name="rh-bridge-poll")
