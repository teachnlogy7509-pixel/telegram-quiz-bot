...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS support_routes_v2 (
...
def save_support_route(
    admin_id: int, admin_message_id: int, user_id: int, user_name: str = ""
):
    conn = get_connection()
    conn.execute(
        """INSERT OR REPLACE INTO support_routes_v2
           (admin_id,admin_message_id,user_id,user_name,created_at)
           VALUES(?,?,?,?,datetime('now'))""",
        (
            int(admin_id),
    ...
    conn.execute(
        """DELETE FROM support_routes_v2 WHERE rowid IN (
           SELECT rowid FROM support_routes_v2
...
def get_support_route(admin_id: int, admin_message_id: int):
    conn = get_connection()
    row = conn.execute(
        """SELECT * FROM support_routes_v2
           WHERE admin_id=? AND admin_message_id=?""",
        (int(admin_id), int(admin_message_id)),
    ).fetchone()
    conn.close()
